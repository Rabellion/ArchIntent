    ...
    <div className="min-h-screen bg-slate-950 pb-24">
      ...
      <div className="pt-12 pb-16 max-w-7xl mx-auto px-4">
        <nav className="flex items-center gap-3 text-slate-400 mb-8">
           ...
           <span className="text-[10px] font-black uppercase tracking-widest text-slate-100">Architect Audit Queue</span>
        </nav>
        
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
           <div>
              <h1 className="text-5xl md:text-6xl font-black italic uppercase tracking-tighter leading-[0.9] text-slate-100">
                Architect <br/><span className="text-indigo-300">Verification</span>
              </h1>
           ...
           <div className="flex items-center gap-4">
              <div className="bg-slate-800 border border-slate-800 rounded-3xl p-6 px-8 flex items-center gap-6 shadow-sm">
                 <div>
                    ...
                    <p className="text-2xl font-black text-slate-100 leading-none">{architects.length} <span className="text-xs uppercase tracking-normal">Pending</span></p>
                 </div>
        ...
        <div className="grid grid-cols-1 gap-8">
          {architects.map((architect) => (
            <div
              ...
              className={`bg-slate-900 rounded-[3.5rem] border border-slate-800 shadow-2xl shadow-slate-200/50 overflow-hidden transition-all duration-700 group hover:border-indigo-500/30 ${
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                  ...
                  <div className="flex-shrink-0 flex flex-col items-center">
                    <div className="relative">
                       ...
                       <div className="absolute -bottom-4 -right-4 w-12 h-12 bg-slate-900 rounded-2xl shadow-xl flex items-center justify-center border border-slate-800">
                          <Fingerprint className="w-6 h-6 text-indigo-300" />
                       </div>
                    </div>
                    <div className="mt-10 px-6 py-2 bg-amber-500/10 text-amber-300 text-[10px] font-black uppercase tracking-[0.2em] rounded-full border-2 border-amber-500/30 shadow-sm">
                      Audit Pending
                    </div>
                  </div>
                  ...
                  <div className="flex-1 space-y-10">
                    <div className="flex flex-wrap items-start justify-between gap-6">
                      <div>
                        <h3 className="text-4xl font-black italic uppercase tracking-tighter text-slate-100 mb-2">{architect.user?.full_name}</h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                      <div className="bg-slate-800 p-6 rounded-[2rem] border border-slate-800">
                        ...
                        <p className="text-lg font-black text-slate-100">{architect.license_number || 'N/A'}</p>
                      </div>
                      <div className="bg-slate-800 p-6 rounded-[2rem] border border-slate-800">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                           <Clock className="w-3.5 h-3.5" /> Experience
                        </p>
                        <p className="text-lg font-black text-slate-100 italic uppercase">{architect.experience_years || 0} Years</p>
                      </div>
                      <div className="bg-slate-800 p-6 rounded-[2rem] border border-slate-800">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                           <Briefcase className="w-3.5 h-3.5" /> Mastery
                        </p>
                        <p className="text-lg font-black text-slate-100 italic uppercase line-clamp-1">{architect.specialization || 'General'}</p>
                      </div>
                    </div>

                    {architect.bio && (
                      <div className="relative p-8 bg-indigo-500/10/30 rounded-[2.5rem] border border-indigo-500/30/50">
                  ...
                  <div className="flex flex-col gap-4 lg:w-72 shrink-0">
                    ...
                    <button
                      ...
                      disabled={processing}
                      className="w-full inline-flex items-center justify-between px-8 py-5 bg-slate-900 text-rose-600 hover:bg-rose-500/10 border-2 border-rose-500/30 rounded-[2rem] transition-all font-black text-[10px] uppercase tracking-widest disabled:opacity-50"
                    >
                      Reject Protocol
      ...
      {confirm.isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xl flex items-center justify-center z-[100] p-6 animate-in fade-in duration-500">
          <div className="bg-slate-900 rounded-[4rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] max-w-lg w-full overflow-hidden border border-white/20 animate-in zoom-in-95 duration-500">
            <div className={`p-12 text-center ${confirm.type === 'approve' ? 'bg-emerald-500/10/50' : 'bg-rose-500/10/50'}`}>
              ...
              <h3 className="text-3xl font-black italic uppercase tracking-tighter text-slate-100">
              ...
              <p className="text-slate-500 mt-3 font-bold uppercase tracking-widest text-[10px]">
                Confirming protocol for <span className="text-slate-100">{confirm.architect?.user?.full_name}</span>
              </p>
            </div>

            <div className="p-12 space-y-8">
              {confirm.type === 'approve' && confirm.architect?.verification_document && (
                <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10/80 px-5 py-4 text-left">
                  ...
                  <Link
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-3 inline-flex text-[10px] font-black uppercase tracking-widest text-indigo-300 hover:text-indigo-300"
                  >
              ...
              {confirm.type === 'reject' && (
                <div className="space-y-4">
                  ...
                  <textarea
                    ...
                    className="w-full px-8 py-6 bg-slate-800 border-2 border-slate-800 rounded-[2.5rem] focus:outline-none focus:border-rose-500 focus:bg-slate-900 transition-all text-sm font-bold text-slate-100 placeholder:text-slate-300 resize-none leading-relaxed"
                    rows={4}
                  />
              ...
              <div className="flex gap-6">
                <button
                  ...
                  disabled={processing}
                  className="flex-1 px-8 py-5 bg-slate-800 text-slate-100 rounded-[2rem] hover:bg-slate-700 transition-all font-black text-[10px] uppercase tracking-widest disabled:opacity-50"
                >
                  Abort
                </button>
                <button
