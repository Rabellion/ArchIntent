...
\begin{enumerate}[label=\arabic*., leftmargin=*, itemsep=2pt]
  ...
  \item To integrate AI-based semantic and voice search functionality
        using WhisperASR and S-BERT, with SpaCy-based keyword extraction
        decoding client intent ahead of matching.
...
decodes a client's spoken or written project brief and matches it
against architect portfolios: voice input is transcribed via the
Whisper API \cite{radford2022whisper}, design intent is extracted with
SpaCy, and the resulting text is embedded with Sentence-BERT for
cosine-similarity ranking \cite{reimers2019sbert}. The full pipeline is
described in Section~\ref{sec:techniques} of Chapter~\ref{ch:methodology}.
...
voice and text-based semantic search over architect portfolios, digital
agreements with signing, and an administrative control panel with
identity verification workflows. The platform integrates the Stripe API
...
Voice transcription is delegated to a hosted Whisper Large V3 endpoint rather
than to a locally-loaded model, for the memory-constraint reasons given
in Section~\ref{subsec:voice-transcription}; the system therefore
requires network access to that third-party service for the voice input
path specifically, while text-based project creation and matching have
no such dependency.
